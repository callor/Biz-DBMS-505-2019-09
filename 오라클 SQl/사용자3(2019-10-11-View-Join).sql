...
SELECT SC.학번, SC.이름, SC.학과코드, DP.d_name,
        SC.국어,  SC.영어, SC.수학
FROM view_sc_st SC -- 주(main) table
    LEFT JOIN tbl_dept DP -- 보조(sub) table
        ON SC.학과코드 = DP.d_num
ORDER BY SC.학번;        
...
(
    SELECT SC.s_num AS 학번, 
            ...
            RANK() 
            OVER 
            ( ORDER BY (SC.s_kor + SC.s_eng + SC.s_math) DESC) AS 석차
    FROM tbl_score SC
        LEFT JOIN tbl_student ST
            ON SC.s_num = ST.st_num 
        LEFT JOIN tbl_dept DP
...
-- 고정된 길이의 값으로 조건하는 것이 결과를 더 확실하게 찾을수 있다.
...
-- 학과 코드가 002부터 004까지
-- 문자열이지만 데이터의 길이 모두 같은 경우
-- 관계산자를 사용한 범위 조회가 가능하다
SELECT 학과코드, 학과이름, 
    SUM(국어) AS 국어총점, 
    SUM(영어) AS 영어총점, 
    SUM(수학) AS 수학총점,
