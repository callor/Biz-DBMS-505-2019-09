-- git hub에 프로젝트를 업로드할때
-- 불필요한 파일이나 비밀번호가 입력된 파일등
-- 업로드를 하지 않아야 될 파일들은
-- git 폴더에 .gitignore 파일을 만들고
-- 파일의 이름, 폴더 이름들을 기록해 주면 된다.
-- test.java 라고 기록하면 test.java 파일은 git hubg에 업로드가 되지 않는다.
...
--1. tbl_student에 보관중인 데이터중에서 이름이 '홍길동'인 리스트만 보고싶다.
-- WHERE 조건 절
SELECT * FROM tbl_student
WHERE st_name = '홍길동' ;
...
-- 학생테이블의 주소칼럼에 저장된 문자열이 '서울'이라는 문자열로 시작되는 데이터를을 보여라
SELECT * FROM tbl_student
...
-- 주소가 익신시, 남원시, 해남군 인 모든 데이터를 보고자 할때
SELECT * FROM tbl_student
...
-- 절대 2개 이상 출력되지 않는다.
-- PK는 절대 중복데이터가 없다.
-- PK는 절대 비어있으면(NULL값) 안된다. : NULL 값이면 안된다.
