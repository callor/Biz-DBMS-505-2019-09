...
SELECT * FROM tbl_address WHERE name = '홍길동' ; -- 홍길동 데이터는 3개가 출려되어
...
-- 이 명령도 개체물결성을 보장하는 데에는 문제가 발생할 소지를 내포한다.
...
-- DBMS를 운영하는 과정에서
-- 만에 하나 재난(실수로 수행명령으로 데이터 변경, 삭제, 천재지변)이 발생했을때
-- 데이터를 복수할수 있는 준비를 해야한다.
...
-- 2. 로그 기록 복구 : INSERT, UPDATE, DELETE 명령이 수행될때
