...
TABLESPACE 생성
이름 : USER4_DB
DATAFILE : '/bizwork/oralce/data/user4.dbf'
초기용량 : 10MB
자동확장 : 10KB
*/
...
-- 보통 DBMS애소 TABLE 등을 TABLESPACE오 옮겨준다.
-- DATA많거나, 하는경우 문제를 일으키는 경우도 있다.
-- 사용중(TABLE..등이 있는)경우에는 가급적 변경하지 않는 것이 좋다.

-- 기존의 TABLESPACE에 있는 TABLE들을 수동으로 다른 TABLESPACE오 옮기기
-- 현재 사용자의 DEFAULT TABLESPACE에 있는 table을 다른 TABLESPACE 옮기기

ALTER TABLE tbl_student MOVE TABLESPACE user4_db ;
-- DEFAULT TABLESPAE를 생성하지 않고 데이터를 저장했을 경우
...
-- oralce 10이상에서
-- TABLESPACE를 통째로 backup하고 다른곳에 옮겨서 사용할수 있도록 하는
-- 방법이 있다
