...
-- 오라클에서는 '' (중간에 빈칸등 아무런 문자열이 없는형태)는 
-- NULL과 같은 의미로 본다.
...
SELECT * FROM tbl_address WHERE address IS NULL; -- adderss 칼럼이 비어있는 레코들들
...
-- chain 칼럼이 값이 001 이면 '가족'이라고 보이고, 아닐경우
...
-- id 칼럼은 PK로 선언되어 있어서
-- 절대 중복값이 있을 수 없다.
-- 그럼에 불구하고 칼럼들을 나열한 것은 의미 없는 코드를 작성한 것이다.
SELECT * FROM tbl_address
