...
일련번호 이기 때문에 일단 칼럼을 NUMBER로 설정을해서
1 ~ 입력순서대로 번호를 부여하겠다.

요구사항2
기존이 입력된 번호와 다른 새로운 번호를 사용해서 데이터를 입력한다.
...
새로 생성하고 싶다. 
  ...
  2. 일번호를 순서대로 자동으로 생성하도록 칼럼을 설정하기
    -- 오라클 11 이하에서는 불가능
...
이제 새로만은 App에서 데이터를 추가할때 SEQUENCE를 사용하고 한다.
...
-- TO_CHAR(숫자,'9999') : 자릿수를 4개로 설정하고, 남는부분은 공백으로 둬라
